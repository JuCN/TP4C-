...
/**
 * Main pour simuler l'application
 * sur console
 * 
 * @author B3431
 */
public class Main {
    ...
    public static void testLivreur() {
        ...
        while (t != 0) {
            t = lireInteger("Souhaitez vous cloturer votre commande(1), "
                    + "consulter votre historique des commandes(2)  ou quitter (0) ?\n", Arrays.asList(0, 1, 2));
            ...
            switch (t) {
                ...
                case 1:
                    ...
                    if (!livreur.getCommandes().isEmpty()) {
                        ...
                        if (coli.getEtat().equals("En cours")) {
                            while (f == null) {
                                f = lireInteger("Avez vous fini cette commande ? oui(1) non(autre)\n" 
                                        + "Clients : " + coli.getClient().getPrenom() + " Restaurant : " + coli.getRestaurant().getDenomination()
                                + " heure de debut : " + coli.getDateDeb(), Arrays.asList(0, 1));
                            }
                            if (f == 1) {
                                smetier.finirCommande(coli, livreur);
                            }
                        } else {
                ...
                case 2:
                    ...
                    for (int i = 0; i < commandes.size(); i++) {
                        ...
                        System.out.println("Clients : " + commandes.get(i).getClient().getPrenom() + " Restaurant : " + commandes.get(i).getRestaurant().getDenomination()
    ...
    public static void testGestionnaire() {
        ...
        while (i == null || i != 0) {
            i = lireInteger("Voulez vous finir une commande d'un drone(1), voir la belle carte(2) "
                    + "ou quitter l'application(0) ?", Arrays.asList(0, 1, 2));
    ...
    public static void visualisationCommandesEnCours() {
        ...
        for (int i = 0; i < commandes.size(); i++) {
            if (commandes.get(i).getEtat().equals("En cours")) {
                System.out.println("Clients : " + commandes.get(i).getClient().getNom() + " Restaurant : " + commandes.get(i).getRestaurant().getDenomination()
    ...
    public static void createCommande(Client client) {
        ...
        commande = smetier.checkCommande(commande, restaurant);
        if (commande == null) {
            System.out.println("Votre commande n'a pas pu être réalisée. Elle est soit trop grosse, soit aucun livreur n'est disponible.");
        } else {
    ...
    public static Date selectDate() {
        ...
        Integer choix = null;
        while (choix == null) {
            choix = lireInteger("Souhaitez vous être livré maintenant(1) ou à une heure précise(2) ?", Arrays.asList(1, 2));
        }
        date = new Date();
        String res = "";
        if (choix != 1) {
